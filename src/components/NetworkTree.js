...
const NetworkTree = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setData(prevData => {
        ...
        const randomNode = findRandomNode(updatedData);
        const newNode = { name: `New Node ${Math.floor(Math.random() * 100)}` };
        if (!randomNode.children) {
          randomNode.children = [];
        }
  ...
  useEffect(() => {
    ...
    const renderTree = () => {
      ...
      .data(root.descendants(), d => d.data.name); // Key by node name to avoid duplicates
